...
public class Reset {
    ...
    private static final String AUTO_RESET_KEY = Constants.PLUGIN_PREFERENCE_PREFIX + ".auto_reset." + Constants.IDE_NAME_LOWER;
    private static final String AUTO_LOGOUT_KEY = Constants.PLUGIN_PREFERENCE_PREFIX + ".auto_logout";
    ...
    public static List<EvalRecord> getEvalRecords() {
        ...
        KeepCondition keepCondition = () -> !isAutoLogout();
        PreferenceRecord[] preferenceValue = new PreferenceRecord[]{
        ...
        for (PreferenceRecord record : preferenceValue) {
        ...
        try {
            ...
            for (String name : Preferences.userRoot().node(DEFAULT_VENDOR).childrenNames()) {
                ...
                getAllPreferenceKeys(Preferences.userRoot().node(DEFAULT_VENDOR + "/" + name + "/" + Constants.IDE_HASH), preferenceList);
            }
            ...
            if (null != METHOD_GET_PRODUCT_CODE) {
                for (IdeaPluginDescriptor descriptor : PluginManager.getPlugins()) {
                    ...
                    getAllPreferenceKeys(Preferences.userRoot().node(DEFAULT_VENDOR + "/" + productCode.toLowerCase()), preferenceList);
                }
            }

            for (String key : preferenceList) {
    ...
    protected static void getAllPreferenceKeys(Preferences preference, List<String> list) throws BackingStoreException {
        String[] childrenNames = preference.childrenNames();
        if (childrenNames.length == 0) {
            for (String key : preference.keys()) {
                list.add(preference.absolutePath() + "/" + key);
            }
            return;
        }

        for (String childName : childrenNames) {
            getAllPreferenceKeys(preference.node(childName), list);
        }
    }
}
